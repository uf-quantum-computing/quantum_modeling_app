...
function Tunneling(a, b, c)

    # INPUT VARIABLES
    V_tunnel = a #intensity of barrier (1-5)
    BarrierThickness = b #thickness of barrier (not the same as intensity but related) (1-5)
    Momentum = c #intensity of wave (1-10)
    ...
    anim2 = @animate for t = 1:size(T)[1]
        ...
        ψ = ψt2D[t]
        n = abs.(ψ.data) .^ 2
        plot(xsample, n, ylims=(0, 0.3), xlabel="Position (x)", ylabel="Probability Density", legend=false, linewidth=3, background_color="black", seriescolor="deepskyblue2")
        plot!(rectangle(BarrierThickness, 1, 13 - (BarrierThickness / 2), 0), color="white", opacity=0.5)
    end

    # ============== generate GIFs ==============
    tunneling_3D_img_string = "tunneling_3D.gif"
    tunneling_2D_img_string = "tunneling_2D.gif"
    gif(anim1, tunneling_3D_img_string, fps=60)
    gif(anim2, tunneling_2D_img_string, fps=60)
end

function Interference(a, b, c)
    # INPUT VARIABLES
    Momentum = a #intensity of wave (1-5)
    Spacing = b #size of the slit spaces (1-10)
    SlitSeparation = c #spacing of the slits relative to the incoming wave (1-5)
    ...
    # ============== generate GIFs ==============
    interference_3D_img_string = "interference_3D.gif"
    interference_2D_img_string = "interference_2D.gif"
...
function Dynamic_Wave(a, b)
    ...
    probDensityAnim = @animate for t = 1:size(T)[1]
        ...
        ψ = ψt[t]
        n = abs.(ψ.data) .^ 2
        plot(xpoints, n, ylims=(0, 0.3), xlabel="Position (x)", ylabel="Probability Density", legend=false, linewidth=3, background_color="black", seriescolor="deepskyblue2")
    end

    wavefuntion_img_string = "wavefunction.gif"
    wavefuntion_probDensity_img_string = "wavefunction_probDensity.gif"
    gif(waveFunctionAnim, wavefuntion_img_string, fps=60)
    gif(probDensityAnim, wavefuntion_probDensity_img_string, fps=60)
end

#Tunneling(Parsers.parse(Int, ARGS[1]), Parsers.parse(Int, ARGS[2]), Parsers.parse(Int, ARGS[3]))